...
public class NetState {
    ...
    /**
     * DNS lookup 을 수행한다.<br>
     * <br>
     * 기본적으로 무거운 작업이다.
     * 특히 네트워크 상태가 좋지 않은 경우 이 메소드는 시간이 오래 걸리므로 주의깊게 사용할 것.<br>
     * <br>
     * <font color=#ff0000>Caution!</font>이 메소드는 ThreadHost 의 스레드에 대하여 wait 를 사용하므로,
    ...
    synchronized public static boolean isConnectable(Context context, final String address, final long timeout) {
		 ...
		 * [현재의 논리]
		 *
		 * Atomic 한 체커를 두고, 별도의 스레드에서 DNS lookup 을 수행한다.
        ...
        // 이 메소드는 스레드 정지 대기를 할텐데, 메인 스레드라면 ANR 의 원인이 된다.
        // 가능하다면 컴파일 타임에 해결해야 하고, 안 되면 어쩔 수 없는 문제다. 로그 모드일 때 경고만 찍어주자.
        if (!CommonConfig.isReleaseVersion() && Thread.currentThread().getId() == 1) {
            LogByCodeLab.w("NetState.isConnectible(): DNS lookup on main thread.");
        }
        LogByCodeLab.v("NetState.isConnectible(): start DNS lookup. address=" + address);
		 ...
		 * 만일 별도 스레드의 lookup 이 아직 안 끝났는데, 이 상태에서 다시 이 메소드에 요청이 들어오면 어떻게 되는 것인가?
        ...
        // 배수 시간 대기, 체커 점검 작업
        int totalTime = 0;
        for (int time = 100; totalTime < timeout; time *= 2) {    // 100ms 부터 배수 시간 대기. 총 대기 시간은 타임아웃 이내.
			/*
			 * 이 루프는 실제로 잘 되는 상황에서는 1~2회 밖에 돌지 않으므로 별 문제가 되지 않는다.
			 * 안 될 때가 문제.
			 * */
            if (checker.get() > 0) {    // 7은 성공
                LogByCodeLab.v("NetState.isConnectible(): success. totalTime=" + totalTime);
                setLoggedConnectivity(address, true);
                return true;
            } else if (checker.get() < 0) {    // -4는 쥬금 ㅠㅜ
                LogByCodeLab.v("NetState.isConnectible(): fail with DNS lookup fail. totalTime=" + totalTime);
                setLoggedConnectivity(address, false);
                return false;
            } else {    // 아마 0
                try {
                    ...
                    LogByCodeLab.e(e);
                    LogByCodeLab.v("NetState.isConnectible(): fail with threading error.");
                    setLoggedConnectivity(address, false);
                    return false;
                }
            }
        }
        LogByCodeLab.v("NetState.isConnectible(): fail with forced timeout. totalTime=" + totalTime);
        setLoggedConnectivity(address, false);
        return false;
    }
    ...
    public static class ConnectivityLog {
         ...
         */
        public boolean isConnectible = false;

        public ConnectivityLog(long elapsedRealtime, boolean isConnectible) {
    ...
    /**
     * {@link #isConnectable(android.content.Context, String, long)} 은 무겁다.
     * 그리고 그 리턴은 물리적 갱신 시간과 연관되기 때문에 시간적으로 긴 유효성을 가진다.
     * 따라서 그 결과를 어딘가에 저장하고 있다가 재활용하면 좋다.
     * (주소, ConnectivityLog) 쌍을 저장해두자.
     */
    private static Hashtable<String, ConnectivityLog> loggedConnectivity = new Hashtable<String, ConnectivityLog>();

    /**
     * {@link #getLoggedConnectivity(String)} 가 돌려주는 정보를 초기화한다.
     */
    public static void resetLoggedConnectivity() {
        loggedConnectivity.clear();
    }

    /**
     * {@link #isConnectable(android.content.Context, String, long)} 의 마지막 호출 결과와 시점을 기억해둔다.
     *
     * @param address 체크 대상
     * @param isConnectible 체크 결과
     */
    private static void setLoggedConnectivity(final String address, final boolean isConnectible) {
        loggedConnectivity.put(address, new ConnectivityLog(SystemClock.elapsedRealtime(), isConnectible));
    }
    ...
    public static ConnectivityLog getLoggedConnectivity(final String address) {
        return loggedConnectivity.get(address);
    }
    ...
    public static int checkLastConnectivity(final String address, long successLogTimeout, long failLogTimeout) {
        ...
        if (connectivityLog != null) {    // 기록이 있고
            if (!connectivityLog.isConnectible) {    // 실패기록이다.
    ...
    public static InetAddress getWifiIpAddress() {
        ...
        try {
            return Inet4Address.getByAddress(intToByteArray(ipAddress));
        } catch (Exception e) {
            return null;
        }
    }
